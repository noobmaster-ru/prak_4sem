...
// Шаблонная для извлечения верхнего элемента из стека.
...
// выяснение какие записи удовлетворяют условиям WHERE
vector<bool> Interpretator::where()
{
    ...
    gp();
    Poliz exp(1000);
    if(pc_el.get_type() == LEX_ALL)   //если выражение содержит ALL - выбираем все
    {
    ...
    {
        // cобираем выражение WHERE в полиз до тех пор, пока не встретим конец предложения или ключевые слова IN, NOT
        while((pc_el.get_type() != POLIZ_SENTENCE) && (pc_el.get_type() != LEX_IN) && (pc_el.get_type() != LEX_NOT)){
            exp.put_lex(pc_el);
            gp();
        }

        moveFirst(table); // в начало таблицы
        
        // если не конец - продолжаем
        if(pc_el.get_type() != POLIZ_SENTENCE)
        {
            int flag = 0;
            
            // если есть NOT - ставим флаг и идем дальше
            ...
            if(pc_el.get_type() == LEX_NUM)
            {
                ...
                {
                    constants.push_back(pc_el.get_value()); //пихаем все константы в вектор
                    gp();
                }
                ...
                while(!afterLast(table))
                {
                    expression(exp); //вычисляем WHERE
                    from_st(args, l);
                    if(flag) // проверка NOT
                    {
                        t = 1;
            ...
            else // строковые константы - далее аналогично
            {
        ...
        else // обрабатываем каждую запись
        {
            while(!afterLast(table))
            {
                expression(exp);
