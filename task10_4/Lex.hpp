...
class Lex
{
    ...
    string name_of_lex; //имя лексемы
public:
...
class Ident
{
    string name;      //имя идентификатора
    bool declare;     //был ли объявлен
    type_of_lex type; //тип
    bool assign;      //было ли присвоено значение
    int value;        //значение
public:
    Ident()
    {
...
class Scanner
{
    ...
    int look(const string& buf, string *list, int n)  //поиск строки в массиве
    {
        int i = 0;
        while (i < n) 
        {
...
public:
    vector<int> counts;           //подсчет количества лексем
    static string TW[], TD[];     //массивы служебных слов и ограничителей
